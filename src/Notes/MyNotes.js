...
function MyNotes() {
  ...
  const addNoteToList = () => {
    ...
    if (textValue) {
      const newLogsList = [...notes, noteValues];
      newLogsList.sort((a, b) => new Date(b.date) - new Date(a.date));
      setNotes(newLogsList);
    } else {
  ...
  const handleCheckBox = (checkedId) => {
    const checkedList = notes.map((itr) => {
      if (itr.id === checkedId) itr.checked = !itr.checked;
      return itr;
    });
    setNotes(checkedList);
    handleSnackbarClick();
  };
  ...
  const handleDelete = (deleteId) => {
    const newList = notes.filter((itr) => itr.id !== deleteId);
    setNotes(newList);
    handleSnackbarClick();
  };

  const saveNotes = () => {
    updateList({...data, notes : notes});
